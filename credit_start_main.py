...
def make_credit_study(prev_agents, trajectory_max_len, episode_n, with_end, with_info, with_late_purchace_penalty):
    reward_system = BoundedEmpiricalCreditStageRewardSystem(with_late_purchase_punishment=with_late_purchace_penalty, config={})
...
def main():
    ...
    study = make_credit_study([tutorial_agent], 100, 1400, with_end=False, with_info=True, with_late_purchace_penalty=False)
    agent = study.agent
    ...
    end_study = make_credit_study(end_agents, 100, 1400, with_end=True, with_info=True, with_late_purchace_penalty=True)
